...
(function () {
  ...
  function create(dirname, connectApp, type, parent) {
    ...
    apps.servers.forEach(function (app) {
      if ('vhosts' === type) {
        connectApp.use(connect.vhost(app.alias, app.server));
        console.info('Loaded ' + app.alias + '.' + app.parent);
      } else if ('mounts' === type) {
  ...
  function createMounts(pathname, app, parent) {
    ...
    newPathname = path.existsSync(typePath) && typePath;
    newPathname = path.existsSync(serverTypePath) && serverTypePath || newPathname;
  ...
  function createVhosts(pathname, app) {
    ...
    if (path.existsSync(typePath)) {
      return create(typePath, app, type);
    }

    if (path.existsSync(serverTypePath)) {
  ...
  function createHelper(pathname) {
    ...
    if (createVhosts(pathname, app) || createMounts(pathname, app)) {
